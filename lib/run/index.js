...
Runner.prototype.initFramework = function() {
  ...
  if (options.parrellel) {
    frameworkBin = path.join(rootDir, '.bin', 'mocha-parallel-tests');
  } else {
    frameworkBin = path.join(rootDir, '.bin', framework);
  }
  ...
  if (options.parrellel) {
    ...
    options.env.RUN_IN_PARRELLEL = true;
  }
